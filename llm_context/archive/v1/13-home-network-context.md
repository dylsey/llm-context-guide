...
**Last Updated:** 2026-01-28  01:28AM UTC
...
**BadNich** — Homelab cybersecurity learning environment, airgapped from YelNet (**planning**)
...
### 2.5 Wireless Infrastructure & Associations
...
### Unused Devices availabe for Lab or Home Netorwork
**Archer AX6000 Wifi 6 Router**
...
|---|---|---|---|---:|---|
| Proton‑WG‑Frankfurt | WireGuard | 10.2.0.2/32 | 149.88.102.46 | 51820 | Not established |
| US_West_SanJose | WireGuard | 10.2.0.10/32 | 79.127.185.222 | 51820 | Not established |
| **US‑Denver** | WireGuard | **10.2.0.20/32** | 95.173.221.65 | 51820 | **Connected** |
...
| USW Ultra 60W | Network | Offline | **10.10.10.112** | — | (shown as) Gateway Fiber Port 3 |
...
- PoE-powered cameras connected to NVR
- NVR connected to network via Archer AX6000
- **Planned:** Move NVR downstairs under house for centralized camera wiring

---

## 8) UniFi Plan (Future State)

### Strategic Goals
1. Verify  proper **network segmentation** (VLANs)
2. Enable **VPN capability** for remote access
3. Isolate **IoT and camera traffic** from trusted devices
4. Create **learning environment** for enterprise networking concepts


**Firewall Policy Goals:**
- IoT/Cameras: **No access** to Trusted VLAN (one-way communication)
- Guest: **Internet only**, isolated from all internal networks
- Homelab: **Isolated** for security testing, controlled access to internet
- Trusted: **Full access** with monitoring

---

## 6) Homelab Infrastructure (Cybersecurity Learning Environment)
...
| **Workbox** | Windows 11 Pro | Hyper-V host, work laptop | Mobile/office |
| **Surface Book Gen 1** | AlmaLinux 9.6/RHEL-Based | Bare metal Linux learning | Mobile |
| **Old Desktop PC** | Need to repurpose with OS for lab use Server | Spare machine | Storage |
| **ClockworkPi uConsole** | Debain-Based OS Fork & Parrot Security OS | Portable Linux experiments and Pen Testing | Mobile |
...
| VM Name | OS | Purpose |
|---------|----|----- ---|
| AlmaLinux  | AlmaLinux 10/RHEL-Based | Linux server/development environment |
...
- Network fundamentals: DHCP → DNS (wants "slow and methodical" deep dive)
...
|----------|--------|-------|
| WireGuard | Under consideration | Modern, performant, simple configuration |
| UniFi VPN | Likely choice | Native integration with UniFi Gateway |
| OpenVPN | Alternative | Industry standard, more complex |

---

## 8) Monitoring & Management Infrastructure
...
### Planned Capabilities (Status: UNKNOWN)
- [ ] Central logging solution (syslog server, SIEM)
- [ ] Network monitoring dashboard (UniFi Network Application, Grafana)
...
**Status:** Unfulfilled / Planning Stage
...
## 10) Future Homelab Expansion Ideas

### Short-Term Goals
- Complete UniFi gateway and switch deployment
- Move Reolink Cameras to outside and connect to NVR
...
- Build malware analysis lab (isolated VM environment)
...
## 11) Louis Art Installation Project (Data Privacy Concept)
...
### Technical Requirements (Early Planning)

**Core Functionality:**
1. **Captive Portal WiFi VLAN**
   - Users connect to dedicated VLAN
...
2. **Data Collection Architecture**
   ...
   - pull leagal data from any scrapable sources on people's devices that opt-in to the experience over wifi
...
- **Platform:** Linux (Raspberry Pi, x86 Linux server) or Windows?
- **Captive Portal Software:** UNKNOWN
- **Data Processing:** Potential -> Python, real-time streaming
- **Audio Engine:** Integration with hardware synth or software synthesizer(JUCE, Max/MSP, Pure Data)
- **Visualization:** Web-based dashboard (React, D3.js?, Touchdesigner with midi controller)
...
**Status:** Conceptual planning phase, technical requirements need refinement and resource gathering

---

## 12) UNKNOWN Items (Network Architecture Gaps)

The following network details are **not yet documented:**

### IP Addressing & Subnets
- [ ] Complete IP addressing scheme for all VLANs
- [ ] DHCP scope ranges and reservations
- [ ] Static IP assignments for critical devices

### Physical Infrastructure
- [ ] Complete switch port mapping (which device on which port)
- [ ] Cable run documentation (lengths, locations, termination points)
- [ ] Rack/equipment mounting plan (if applicable)
...
### Management & Access
- [ ] Admin access credentials strategy (password manager, secrets management)
...
- **YelNet is in transition:** Current state is setup but not fully finalized
...
- **UniFi bias:** Dylan is planning UniFi ecosystem, prefer UniFi-specific guidance when relevant
